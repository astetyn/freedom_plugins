...
public class SetColorCommand extends Command {
	...
	public void execute(CommandSender commandSender, String[] args) {
		...
		if(tokenCount==0) {
			pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.WARNING + ChatColor.RED + "/setcolor <index>"));
			...
			if(!b||args[0].length()>1) {
				pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.COLOR + ChatColor.RED + "This color is not supported."));
				return;
			}
			
			pd.setColorIndex(index);
			
			pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.COLOR + ChatColor.GREEN + "Color successfuly changed to: "
			...
			if(!pp.hasPermission(Permissions.SET_COLOR_OTHERS)) {
				pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.INSUFFICIENT_PERMISSION));
				return;
			}
			...
			for(GPlayer pdd : Main.gPlayers) {
				if(pdd.getProxiedPlayer().equals(ProxyServer.getInstance().getPlayer(name))) {
					pdd.setColorIndex(index);
					pdd.getProxiedPlayer().sendMessage(TextComponent.fromLegacyText(ChatUtils.COLOR + ChatColor.GREEN + "Color successfuly changed to: "
