...
public class MuteCommand extends Command {
	...
	public void execute(CommandSender commandSender, String[] args) {
		...
		if(tokenCount==0||tokenCount>2) {
			pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.WARNING + ChatColor.RED + "/mute <nick>"));
			...
			if(mutedPlayerData.isMuted()) {
				pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.WARNING + ChatColor.GRAY + "Player is already muted. For unmute - /unmute <nick>"));
				return;
			}
			...
			pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.SILENCE + ChatColor.GRAY + "Player is silenced. His messages will not be shown."));
			mutedPlayerData.getProxiedPlayer().sendMessage(TextComponent.fromLegacyText(ChatUtils.SILENCE +
