...
public class UnmuteCommand extends Command {
	...
	public void execute(CommandSender commandSender, String[] args) {
		...
		if(tokenCount!=1) {
			pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.WARNING + ChatColor.RED + "/unmute <nick>"));
			return;
		}
		...
		if(unmutedPlayerData.isMuted()) {
			...
			pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.SILENCE + ChatColor.GRAY + "Player was unmuted. His messages will be shown."));
			unmutedPlayerData.getProxiedPlayer().sendMessage(TextComponent.fromLegacyText(ChatUtils.SILENCE +
			...
			pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.WARNING + ChatColor.RED + "Player is not silenced. Please use - /mute <nick>"));
		}
	}
}
