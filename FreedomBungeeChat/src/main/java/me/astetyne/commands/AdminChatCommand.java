	...
	public void execute(CommandSender commandSender, String[] args) {
		...
		if(tokenCount==0) {
			if(pd.hasActivedAdminChat()) {
				pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.ADMIN_CHAT + ChatColor.RED + "deactivated."));
				pd.setActivedAdminChat(false);
			}else {
				pp.sendMessage(TextComponent.fromLegacyText(ChatUtils.ADMIN_CHAT + ChatColor.GREEN + "activated."));
