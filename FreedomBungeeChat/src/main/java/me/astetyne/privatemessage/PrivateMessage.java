package me.astetyne.privatemessage;
...
public class PrivateMessage {
	...
	public void wantsToSendMessage() {
		...
		if(ProxyServer.getInstance().getPlayer(receiverName)==null) {
			sender.sendMessage(TextComponent.fromLegacyText(ChatUtils.PLAYER_NOT_ONLINE));
			return;
		}
		...
		for(String ignoreName : receiverData.getIgnoredPlayersNames()) {
			if(ignoreName.equals(sender.getName())) {
				sender.sendMessage(TextComponent.fromLegacyText(ChatUtils.WARNING + "Receiver has blocked your messages."));
				return;
			}
		}
	...
	private void sendMessage(GPlayer gSender, GPlayer gReceiver) {
		...
		receiver.sendMessage(TextComponent.fromLegacyText(ChatUtils.PRIVATE_MESSAGE+ChatColor.GRAY+sName+ChatColor.GOLD+" > "
		+ ChatColor.GRAY + "me" + " > " + ChatColor.WHITE + message));
		
		sender.sendMessage(TextComponent.fromLegacyText(ChatUtils.PRIVATE_MESSAGE + ChatColor.GRAY + "me" + ChatColor.GOLD + " > "
		+ ChatColor.GRAY + rName + " > " + ChatColor.WHITE + message));
		
		for(GPlayer pd : Main.gPlayers) {
			if(pd.hasSocialSpy()) {
				if(!pd.equals(gSender)&&!pd.equals(gReceiver)) {
					pd.getProxiedPlayer().sendMessage(TextComponent.fromLegacyText(ChatUtils.SOCIAL_SPY + ChatColor.GRAY + sName + ChatColor.GOLD + " > "
